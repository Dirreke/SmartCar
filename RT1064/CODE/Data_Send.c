...
void Variable_update(void)
{
  ...
  Variable[2] = Picture_Failure;//curve_state * 100 + Mid_slope; //Allwhitestart * 100 + Allwhiteend; //EM_Value_1;//Cam_offset;
  ...
  Variable[7] = loop_distance * 0.001; //PID_CAR_STRAIGHT_CAM.P;//EM_Value_2 + EM_Value_3;
  if (Road == 0)
  {
  ...
  Variable[13] = icm_gyro_y_w;      //MotorOut1_add;
  Variable[14] = icm_gyro_y_angle;  //MotorOut2_add;//speed_change_flag;//car_center_dias;   //100 + Lef_circle * 10 + Rig_circle; /*Rig_circle;*/ //Road6_flag ;//CarSpeed2;
